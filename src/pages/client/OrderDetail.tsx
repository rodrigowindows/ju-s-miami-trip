    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="px-4 pt-4 pb-24 space-y-6">
              ...
              {productImages.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 rounded-full p-1 shadow disabled:opacity-30"
                  >
                    <ChevronLeft size={16} />
                  </button>
                  <button
                    onClick={() => setPhotoIndex(Math.min(productImages.length - 1, photoIndex + 1))}
                    disabled={photoIndex === productImages.length - 1}
                    className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 rounded-full p-1 shadow disabled:opacity-30"
                  >
                    ...
                    {productImages.map((_, i) => (
                      <div
                        ...
                        className={`w-1.5 h-1.5 rounded-full transition-colors ${
                          i === photoIndex ? "bg-violet-600" : "bg-muted-foreground/30"
                        }`}
                      />
                    ))}
                  </div>
              ...
              return (
                <div key={step.status} className="flex gap-3">
                  ...
                  <div className={`pb-4 ${!isDone ? "opacity-40" : ""}`}>
                    <p className={`text-sm font-medium ${isCurrent ? "text-violet-700" : "text-foreground"}`}>
                      {step.title}
                    </p>
                    {event && (
        ...
        <div className="space-y-3">
          ...
          >
            <a
              href={`https://wa.me/5561999999999?text=${whatsappText}`}
              target="_blank"
          ...
          {order.status === "chegou_brasil" && (
            <Button
              ...
              variant="outline"
              className="w-full rounded-full gap-2 border-violet-200 text-violet-700 hover:bg-violet-50"
            >
              <Calendar size={16} />
              Agendar entrega
            </Button>
        ...
        <DialogContent className="max-w-sm mx-auto rounded-2xl">
          <DialogHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Escolha a data e turno para receber seu pedido
            </p>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {([
                  ...
                  <button
                    key={period.key}
                    onClick={() => setSchedulePeriod(period.key)}
                    className={`p-3 rounded-xl border text-center transition-colors ${
                      schedulePeriod === period.key
