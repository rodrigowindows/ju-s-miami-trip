...
const TripDetail = () => {
  ...
  const pct =
    trip.max_weight_kg > 0
      ? (trip.allocated_weight_kg / trip.max_weight_kg) * 100
      : 0;
  ...
  const handleUpdate = async () => {
    try {
      await updateTrip.mutateAsync({
        id: trip.id,
        ...editForm,
        max_weight_kg: Number(editForm.max_weight_kg),
      });
  ...
  const handleAllocate = async (orderId: string) => {
    try {
      ...
      const message = err instanceof Error ? err.message : "Erro ao alocar";
      toast({ title: "Erro", description: message, variant: "destructive" });
    }
  };
              ...
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {canDelete
                    ...
                    : "Remova todos os pedidos alocados antes de excluir."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="font-body text-xl">{trip.code}</CardTitle>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="font-medium">
                {format(new Date(trip.departure_date + "T12:00:00"), "dd/MM/yyyy", {
                  locale: ptBR,
                })}
              </span>
            </div>
            <div>
              ...
              <span className="font-medium">
                {format(new Date(trip.arrival_date + "T12:00:00"), "dd/MM/yyyy", {
                  locale: ptBR,
                })}
              </span>
            </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="font-body text-lg">
              Pedidos Alocados ({orders.length})
            </CardTitle>
            <Button
              size="sm"
              className="gap-2"
        ...
        <CardContent>
          {orders.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-6">
              Nenhum pedido alocado nesta viagem.
            </p>
          ) : (
            <Table>
              <TableHeader>
      ...
      <Dialog open={allocateOpen} onOpenChange={setAllocateOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione um pedido sem viagem atribuida
            </DialogDescription>
          </DialogHeader>
          <div className="max-h-80 overflow-y-auto space-y-2">
            {unassigned && unassigned.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-6">
                Nenhum pedido disponivel para alocacao.
              </p>
            )}
          </div>
        </DialogContent>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, flight_number: e.target.value })
                  }
                />
              </div>
            </div>
            <div>
              ...
              <Input
                value={editForm.traveler_name}
                onChange={(e) =>
                  setEditForm({ ...editForm, traveler_name: e.target.value })
                }
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdate} disabled={updateTrip.isPending}>
              {updateTrip.isPending ? "Salvando..." : "Salvar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
